...
const useFetchFund = ({ keywordFund }) => {
    const [ gifUrl, setGifUrl] = useState("");

    const fetchGifs = async () => {
        try {
            // response from ghipy Api
            const response = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${VITE_GiPHY_API_KEY}&q=${keywordFund.split(" ").join("")}&limit=1`);
    ...
    useEffect(() => {
      if(keywordFund) fetchGifs();      
    }, [keywordFund])   // useEffect happens whenever the keyword changes...

    return gifUrl;
};


export default useFetchFund;
